        ...
        <div class="bg-white" style="border-radius: 20px;">
            <div class="p-3">
                <h3 class="page-title">
                    ...
                    <span>{{ ($duplicate_contract ?? '' == 1) ? 'Duplicate Contract' : ((isset($data['id'])) ? 'Update Contract' : 'Add Contract') }}</span>
                </h3>
            </div>
            <div class="container" id="home">
                <form action="contractStore" id="formData" method="post">
                    <div class="row">
                        @csrf
                        <div class="col-lg-{{ ($user->role == user_roles('3')) ? (($user->role == user_roles('2')) ? '3' : '4') : '3' }} col-md-6 col-sm-12  my-2">
                            <label for="c_st_date">@lang('Start Date')</label>
                            <input required type="date" name="start_date" id="c_st_date" value="{{ ($duplicate_contract ?? '' == 1) ? date('Y-m-d') : ((isset($data['id'])) ? $data['start_date'] : date('Y-m-d') ) }}" min="{{ ($duplicate_contract ?? '' == 1) ? date('Y-m-d') : ((isset($data['id'])) ? $data['start_date'] : date('Y-m-d') ) }}" class="form-control">
                            <span id="c_st_date_error" class="error-message text-danger"></span>
                            <input type="hidden" name="id" id="q_id" value="{{ ($duplicate_contract == 1) ? '' : ((isset($data['id'])) ? $data['id'] : '') }}" />
                            <input type="hidden" name="sadmin_id" id="sadmin_id" value="{{ $sadmin_id ?? ''}}" />
                        </div>

                        <div class="col-lg-{{ ($user->role == user_roles('3')) ? (($user->role == user_roles('2')) ? '3' : '4') : '3' }} col-md-6 col-sm-12  my-2">
                            <label for="c_ex_date">@lang('End Date')</label>
                            <input required type="date" name="end_date" id="c_ex_date" value="{{ ($duplicate_contract ?? '' == 1) ? date('Y-m-d') : ((isset($data['id'])) ? $data['end_date'] : date('Y-m-d') ) }}" min="{{ ($duplicate_contract ?? '' == 1) ? date('Y-m-d') : ((isset($data['id'])) ? $data['end_date'] : date('Y-m-d') ) }}" class="form-control">
                            <span id="c_ex_date_error" class="error-message text-danger"></span>
                        </div>

                        @if(isset($user->role) && ($user->role == user_roles('1')))
                        <div class="col-lg-{{ $user->role == user_roles('2') ? '3' : '3' }} col-md-6 col-sm-12 my-2">
                        ...
                        <div class="col-lg-{{ ($user->role == user_roles('3')) ? (($user->role == user_roles('2')) ? '4' : '4') : '3' }} col-md-6 col-sm-12">
                            <label class="mt-2" for="client_name">@lang('lang.client_name')</label>
                        ...
                        <div class="col-lg-3 col-md-6  col-sm-12">
                            <label for="service_id">@lang('lang.contract_category')</label>
                            <select required name="service_id" id="service_id" class="form-select">
                                <option disabled selected> @lang('select contract category')</option>
                        ...
                        <div class="col-lg-3 col-md-6  col-sm-12">
                            <label for="service_id">@lang('Contract Location')</label>
                        ...
                        <div class="col-lg-2 col-md-6 col-sm-12">
                            <label for="q_amount">@lang('lang.contract_amount')</label>
                        ...
                        <div class="col-lg-2 col-md-6 col-sm-12">
                            <label for="q_file">@lang('Contract File ')</label>
                        ...
                        <div class="row justify-content-end mt-2  ">
                            ...
                            <div class="col-lg-2 col-md-6 col-sm-12 mb-5 mb-md-5 mb-lg-4 text-right">
                                <button type="submit" id="btn_save_quotation" class="btn btn-block  text-white" style="background-color: #184A45FF; border-radius: 8px;">
                                    ...
                                    <span id="text">@lang('Save Contract')</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#btn_save_quotation').click(function(event) {
            var c_st_Date = $('#c_st_date').val();
            var c_ex_Date = $('#c_ex_date').val();
            ...
            if (c_st_Date === '') {
                $('#c_st_date_error').text('*Please enter a contract start date.');
                event.preventDefault();
            }

            if (c_ex_Date === '') {
                $('#c_ex_date_error').text('*Please enter a contract end date.');
                event.preventDefault();
            }
        ...
        $('#c_st_date').on('input', function() {
            $('#c_st_date_error').text('');
        });

        $('#c_ex_date').on('input', function() {
            $('#c_ex_date_error').text('');
        });
